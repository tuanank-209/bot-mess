...
function formatDeltaEvent(m) {
    ...
    switch (m.class) {
        ...
        case "ParticipantLeftGroupThread":
            ...
            break;
        case 'ApprovalQueue':
            logMessageType = "ApprovalQueue";
            logMessageData = {
                action: m.action, // Hành động xảy ra (ví dụ: REQUESTED)
                // inviterFbId: m.inviterFbId, // ID của người mời
                USERID: m.recipientFbId, // ID của người nhận lời mời
                requestSource: m.requestSource, // Nguồn yêu cầu (ví dụ: ADD)
                requestTimestamp: m.requestTimestamp, // Thời gian yêu cầu
                threadID: m.messageMetadata?.threadKey?.threadFbId, // ID của nhóm
                messageID: m.messageMetadata?.messageId, // ID của tin nhắn liên quan
                senderID: m.messageMetadata?.actorFbId, // ID của người thực hiện hành động
            };
            text_new = `` // this is logMessageBody =)))))
            break;
    ...
    return {
        ...
        logMessageType: logMessageType,
        logMessageData: logMessageData,
        logMessageBody: m.messageMetadata.adminText || text_new,
